...
class Angles:
    def __init__(self, h, k, a):
        # self.h = h * math.radians(1)
        # self.k = k * math.radians(1)
        # self.a = a * math.radians(1)
        c = math.pi / 180.0
        self.h = h * c
        self.k = k * c
        self.a = a * c
    ...
    def as_string(self):
        return f"[{self.h:.5f}] [{self.k:.5f}] [{self.a:.5f}]"
...
class WalkData:
    ...
    d = [[1, 0, -15.2, -2.29, 94.89, 46.7, 6.74, -21.91, -2.4, 2.39, 36.03],
         ...
         [8, 0.1, -6.9, 2.76, -18.7, 67.3, -0.53, -81.99, 16.8, 2.95, -30.3],
         [9, 0.114, -4.8, 2.38, -30.23, 66.4, -1.66, -75.17, 19, 2.48, -33.55],
         [10, 0.129, -3, 1.9, -34.13, 64.6, -2.68, -67.64, 20.8, 1.99, -35.01],
         [11, 0.143, -1.7, 1.41, -32.12, 62, -3.6, -60.31, 22.2, 1.48, -35.53],
         ...
         [17, 0.229, 0.9, 0.14, -0.56, 34.5, -6.99, -18.05, 23.1, -0.77, -16.1],
         ...
         [28, 0.386, -0.4, -2.24, -38.3, -0.6, 1.76, 40.55, 12.8, -0.81, 7.93],
         [29, 0.4, -2.5, -2.63, -7.99, 1.1, 2.2, 24.37, 12.2, -0.69, 9.91],
         [30, 0.415, -4.7, -2.47, 30.12, 3, 2.46, 16.62, 11.7, -0.52, 13.66],
         [31, 0.429, -6.5, -1.77, 59.6, 5.1, 2.67, 12.93, 11.4, -0.3, 16.99],
         ...
         [35, 0.486, -6, 1.59, 24.33, 14, 2.07, -44.04, 11.6, -0.05, -22.88],
         [36, 0.5, -4.6, 1.77, 4.14, 15.4, 1.34, -53.13, 11.4, -0.47, -32],
         [37, 0.515, -3.1, 1.71, -8.73, 16.2, 0.55, -52.57, 10.8, -0.96, -31.67],
         [38, 0.529, -1.8, 1.52, -13.85, 16.3, -0.17, -44.3, 9.8, -1.38, -23.59],
         ...
         [42, 0.586, 2.1, 0.96, -6.79, 13.2, -1.18, 5.07, 4.5, -1.4, 16.74],
         [43, 0.601, 2.8, 0.84, -10.34, 12.3, -1.06, 8.5, 3.5, -1.13, 17.58],
         [44, 0.615, 3.4, 0.66, -12.36, 11.4, -0.94, 7.49, 2.7, -0.89, 15.03],
         [45, 0.629, 3.9, 0.49, -9.52, 10.7, -0.85, 4.99, 2, -0.7, 11.6],
         [46, 0.643, 4.2, 0.39, -3.26, 10.1, -0.8, 3.03, 1.5, -0.56, 8.39],
         ...
         [51, 0.715, 6.4, 0.51, -11.5, 7, -0.72, 1.21, -0.5, -0.7, -7.39],
         [52, 0.729, 6.7, 0.3, -14.34, 6.4, -0.69, 4.06, -1.2, -0.79, -4],
         ...
         [60, 0.844, 5.9, -1.04, -37.44, 8.9, 2.18, 37.19, -5.2, -0.4, 1.24],
         ...
         [64, 0.901, -1.4, -3.4, -43.21, 19.6, 4.41, 39.95, -6.2, -0.08, 11.23],
         [65, 0.915, -4.4, -4.03, -38.18, 23.5, 4.97, 37.59, -6.2, 0.1, 14.31],
         [66, 0.929, -8, -4.5, -17.72, 27.8, 5.48, 32.25, -6, 0.33, 18.85],
         [67, 0.944, -11.8, -4.54, 18.41, 32.4, 5.9, 23.44, -5.7, 0.64, 24.81],
         [68, 0.958, -15.4, -3.97, 59.42, 37.4, 6.15, 10.87, -5, 1.04, 31.17],
         [69, 0.972, -18.3, -2.84, 91.15, 42.5, 6.21, -4.8, -4, 1.53, 36.24],
         ...
         [75, 1.058, -14, 2.65, -3.93, 65.4, 1.92, -80.71, 10.2, 3.41, -12.63],
         [76, 1.072, -11.8, 2.5, -13.24, 66.5, 0.73, -83.96, 12.9, 3.13, -25.25],
         [77, 1.087, -9.9, 2.28, -16.29, 66.6, -0.48, -82.7, 15.3, 2.69, -33.71],
         ...
         [84, 1.187, -2, 0.55, -10.79, 45.2, -6.02, -35.35, 22.4, 0.31, -15.62],
         [85, 1.201, -1.6, 0.41, -7.88, 40, -6.52, -32.9, 22.6, 0.06, -18.75],
         [86, 1.215, -1.3, 0.32, -3.38, 34.5, -6.96, -26.57, 22.5, -0.23, -21.73],
         [87, 1.23, -1.1, 0.31, 3.27, 28.6, -7.28, -14.15, 22.2, -0.56, -23.63],
         [88, 1.244, -0.8, 0.42, 9.14, 22.6, -7.37, 4.76, 21.6, -0.9, -23.22],
         ...
         [94, 1.33, 1.7, -0.28, -44.2, -2.6, -1.15, 101.58, 14.9, -1.19, 8.22],
         [95, 1.344, 1.2, -1.02, -54.99, -3, 0.22, 85.39, 14, -1.09, 6.14],
         [96, 1.358, 0, -1.85, -53.1, -2.3, 1.3, 63.88, 13.1, -1.01, 5.89],
         [97, 1.373, -1.8, -2.53, -31.93, -0.8, 2.05, 43.96, 12.3, -0.92, 8.35],
         [98, 1.387, -4.1, -2.77, 4.96, 1.1, 2.55, 29.9, 11.6, -0.78, 12.18],
         [99, 1.401, -6.4, -2.39, 43.33, 3.4, 2.9, 20.18, 11.1, -0.57, 14.65],
         [100, 1.416, -8, -1.53, 67.62, 5.9, 3.13, 9.21, 10.7, -0.36, 13.2],
         [101, 1.43, -8.9, -0.46, 71.62, 8.5, 3.17, -6.47, 10.5, -0.2, 6.83],
         [102, 1.444, -8.8, 0.52, 59.88, 11, 2.94, -24.49, 10.4, -0.16, -2.99],
         [103, 1.459, -8, 1.25, 41.13, 13.3, 2.47, -39.25, 10.2, -0.28, -12.86],
         [104, 1.473, -6.7, 1.7, 22.1, 15.1, 1.82, -46.9, 9.9, -0.53, -19.17],
         [105, 1.487, -5.2, 1.89, 5.23, 16.3, 1.13, -47.03, 9.4, -0.83, -19.53],
         [106, 1.501, -3.7, 1.85, -10.1, 16.9, 0.48, -40.67, 8.5, -1.09, -13.02]]
    ...
    def __init__(self):
        ...
        for i in range(len(data)):
            if 26 < i < 96:
                h = float(data[i][self.IDX_H])
                k = float(data[i][self.IDX_K])
                a = float(data[i][self.IDX_A])
                self.list.append(Angles(h, k, a))
        print('Initialized WalkData Instance')

    def print_data(self):
        for i, angle in enumerate(self.list):
            print(f"[{i}] [{angle.as_string()}]")


if __name__ == '__main__':
    walk_data = WalkData(d)
    walk_data.print_data()
